...
%                  GMR  SSR  ETSIT  UPM
%
% Programa básico de procesado de un CW-RADAR
% Un solo canal
%
...
% número de muestras
np=256;
% número de muestras de STFT
zp=np*10;
% número de muestras de la FFT, entre np y zp se rellenan con ceros
% Zero-Padding
paso=32;
M=N/paso;
% número de medidas (sin solapamiento si paso=np), 
% número de slots
...
%    axis([0 50 max(20*log10(abs(Yfft)))-80 max(20*log10(abs(Yfft)))])
  ...
  %El proceso de medida consiste en calcular el valor máximo de la FFT
  %Se utiliza la como resultado final el primero de las dos posible
  %soluciones
  %No se tiene información de signo
